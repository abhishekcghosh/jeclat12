...
if (xmlhttpA.readyState==4)
  {
	  ...
	  {
		  clearFormA();
		  alert("Thank you for your time! You may want to see who others have registered. To do so, follow the 'View a List of Who Have' link.");
		  doAlumniYearView();
	  }
	  else
	  {
...
function doAlumniRegister()
{
	...
	addr=document.getElementById("af_address").value;
	...
	if (name.length==0 || year.length==0 || dept.length==0 || comp.length==0 || addr.length==0 || phon.length==0 || emai.length==0 || stat.length==0) { alert("One or more fields empty!"); return; }
	...
	var params="na="+name+"&ni="+nick+"&ye="+year+"&de="+dept+"&co="+comp+"&ad="+addr+"&ph="+phon+"&em="+emai+"&st="+stat+"&ra="+Math.random();
...
function clearFormA()
{
	...
	document.getElementById("af_address").value="";
